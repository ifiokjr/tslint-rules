...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public static metadata: Lint.IRuleMetadata = {
    ruleName: 'custom-no-implicit-dependencies',
    description:
...
/* TODO: fix this as it doesn't capture most failures */
// function moduleResolves(
//   importModule: string,
...
function walk(ctx: Lint.WalkContext<Options> /*, program: ts.Program */) {
  ...
  for (const name of findImports(ctx.sourceFile, ImportKind.All)) {
    if (!ts.isExternalModuleNameRelative(name.text)) {
      ...
      // const isValidModule = moduleResolves(packageName, ctx, program);
      if (
...
// function findTsConfig(current: string): string | undefined {
//   let prev: string;
//   do {
...
//   } while (prev !== current);
//   return undefined;
//   ts.nodeModuleNameResolver(moduleName, containingFile, compilerOptions, host)
// }
