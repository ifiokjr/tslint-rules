...
export class Rule extends Lint.Rules.TypedRule {
  public static metadata: Lint.IRuleMetadata = {
    ruleName: 'custom-no-unused-variable',
...
function parseOptions(
  options: Array<string | { [key: string]: string | null | undefined }>,
): Options {
  ...
  for (const o of options) {
    if (typeof o === 'object') {
      const ignore = o[OPTION_IGNORE_PATTERN];
      if (ignore) {
  ...
  forEachImport(sourceFile, importNode => {
    ...
    function removeAll(errorNode: ts.Node, failure: string): void {
      ...
      utils.forEachToken(
        importNode,
        token => {
          ts.forEachTrailingCommentRange(ctx.sourceFile.text, token.end, (_, commentEnd, __) => {
            end = commentEnd;
          });
        },
        ctx.sourceFile,
...
function isImportUsed(
  ...
  return (
    ts.forEachChild(sourceFile, function cb(child: ts.Node): boolean | undefined {
